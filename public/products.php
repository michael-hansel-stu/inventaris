                ...
                <?php if (!empty($products_list)): ?>
                    <?php foreach ($products_list as $product): ?>
                        <tr class="border-b border-gray-200 hover:bg-slate-50 transition duration-100">
                            ...
                            <td class="px-5 py-4 whitespace-nowrap text-sm"><?php echo isset($product['price']) ? formatRupiah($product['price']) : 'N/A'; ?></td>
                            <td class="px-5 py-4 whitespace-nowrap text-sm"><?php echo sanitizeInput($product['quantity']); ?></td>
                            ...
                            <td class="px-5 py-4 whitespace-nowrap text-sm">
                                ...
                                <form method="POST" action="products.php" class="inline-block" onsubmit="return confirm('Apakah Anda yakin ingin menghapus produk \'<?php echo htmlspecialchars(addslashes($product['name']), ENT_QUOTES); ?>\'? Tindakan ini tidak dapat dibatalkan.');">
...
<div id="addProductModal" class="modal fixed inset-0 bg-gray-600 bg-opacity-75 overflow-y-auto h-full w-full items-center justify-center z-50">
    <div class="relative mx-auto p-6 border w-full max-w-lg shadow-lg rounded-md bg-white transform transition-all sm:my-8 opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" 
        ...
        <div class="flex justify-between items-center pb-3 border-b mb-4">
            <p class="text-2xl font-bold text-gray-800" id="modal-headline-add">Tambah Produk Baru</p>
        ...
        <form method="POST" action="products.php" id="addProductFormInternal">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <input type="text" name="name" id="add_name" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required>
                </div>
                <div>
                    <label for="add_description" class="block text-sm font-medium text-gray-700 mb-1">Deskripsi</label>
                    <textarea name="description" id="add_description" rows="3" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"></textarea>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label for="add_price" class="block text-sm font-medium text-gray-700 mb-1">Harga (Rp) <span class="text-red-500">*</span></label>
                        <input type="number" name="price" id="add_price" min="0" step="1" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required>
                    </div>
                    <div>
                        <label for="add_quantity" class="block text-sm font-medium text-gray-700 mb-1">Jumlah Stok <span class="text-red-500">*</span></label>
                        <input type="number" name="quantity" id="add_quantity" min="0" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required>
                    </div>
                </div>
                <div>
                    <label for="add_unit" class="block text-sm font-medium text-gray-700 mb-1">Satuan (mis: pcs, kg) <span class="text-red-500">*</span></label>
                    <input type="text" name="unit" id="add_unit" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required>
                </div>
            </div>
            <div class="flex justify-end pt-6 space-x-3 border-t mt-6">
                ...
                <button type="submit" class="px-4 bg-blue-600 py-2 rounded-lg text-white hover:bg-blue-700 font-medium transition duration-150">Simpan Produk</button>
            </div>
        </form>
    </div>
</div>
...
<div id="editProductModal" class="modal fixed inset-0 bg-gray-600 bg-opacity-75 overflow-y-auto h-full w-full items-center justify-center z-50">
    <div class="relative mx-auto p-6 border w-full max-w-lg shadow-lg rounded-md bg-white transform transition-all sm:my-8 opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95"
         ...
         <div class="flex justify-between items-center pb-3 border-b mb-4">
            <p class="text-2xl font-bold text-gray-800" id="modal-headline-edit">Edit Produk</p>
        ...
        <form method="POST" action="products.php" id="editProductFormInternal">
            ...
            <input type="hidden" name="product_id_edit" id="edit_product_id">
            <div class="space-y-4">
                <div>
                    <label for="edit_name" class="block text-sm font-medium text-gray-700 mb-1">Nama Produk <span class="text-red-500">*</span></label>
                    <input type="text" name="name_edit" id="edit_name" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required>
                </div>
                <div>
                    <label for="edit_description" class="block text-sm font-medium text-gray-700 mb-1">Deskripsi</label>
                    <textarea name="description_edit" id="edit_description" rows="3" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"></textarea>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label for="edit_price" class="block text-sm font-medium text-gray-700 mb-1">Harga (Rp) <span class="text-red-500">*</span></label>
                        <input type="number" name="price_edit" id="edit_price" min="0" step="1" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required>
                    </div>
                    <div>
                        <label for="edit_quantity" class="block text-sm font-medium text-gray-700 mb-1">Jumlah Stok <span class="text-red-500">*</span></label>
                        <input type="number" name="quantity_edit" id="edit_quantity" min="0" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required>
                    </div>
                </div>
                <div>
                    <label for="edit_unit" class="block text-sm font-medium text-gray-700 mb-1">Satuan <span class="text-red-500">*</span></label>
                    <input type="text" name="unit_edit" id="edit_unit" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required>
                </div>
            </div>
            <div class="flex justify-end pt-6 space-x-3 border-t mt-6">
...
// Koneksi database ditutup di akhir skrip utama jika dibuka di awal
if (isset($conn) && $conn instanceof mysqli) {
    $conn->close();
}
?>
<script>
    ...
    function openModal(modalId) {
        ...
        if (modal) {
            ...
            // Tambahkan class untuk animasi jika menggunakan Tailwind transitions
            ...
            if (modalId === 'addProductModal') {
                // Reset form tambah jika dibuka kembali
    ...
    function closeModal(modalId) {
        ...
        if (modal) {
            ...
            }, 300); // Sesuaikan dengan durasi transisi Tailwind Anda
        }
    }

    function openEditModal(id, name, description, price, quantity, unit) {
        const modal = document.getElementById('editProductModal');
        if(modal) {
            modal.querySelector('input[name="product_id_edit"]').value = id;
            modal.querySelector('input[name="name_edit"]').value = name;
            modal.querySelector('textarea[name="description_edit"]').value = description;
            modal.querySelector('input[name="price_edit"]').value = price;
            modal.querySelector('input[name="quantity_edit"]').value = quantity;
            modal.querySelector('input[name="unit_edit"]').value = unit;
    ...
    function closeFlashMessage(elementId) {
        ...
        if (flashMessage) {
            // Anda bisa menambahkan animasi fade out dengan Tailwind atau CSS kustom di sini
            flashMessage.style.opacity = '0';
            setTimeout(() => {
                flashMessage.style.display = 'none';
            }, 300); // Sesuaikan durasi dengan animasi CSS Anda
        }
    }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (successFlash) {
            ...
            }, 3000); // Hilang setelah 5 detik
        }
        if (errorFlash) {
            setTimeout(() => {
                closeFlashMessage('flashMessageError');
            }, 3000); // Error mungkin perlu waktu lebih lama untuk dibaca
        }
    });
